...
import { gitReposJs,  options } from '../repos';
import { paths } from '../helpers/path';

const { repo  } = gitReposJs;
const git = simpleGit(options);



/**
 * generated new project with javascript 
 */

export const GenerateProjectJS = async (prop:GlobalOptions) => {
  const { name, typeServer } = prop;
  const repoName = 'server-setups-js';
